...
class UpdatePaper extends Component {
    ...
    submit = (event) => {
        ...
        UsersServiceComponent.listUserById(this.state.editorId).then(res=>{
            ...
            let paperObj={paperId: this.state.paperId, editor: this.state.userObj, price: this.state.price, publishDate: this.state.publishDate, newsList: this.state.newsData}
        PaperServiceComponent.updatePaper(paperObj).then(res1=>{
    ...
    componentDidMount() {
        ...
        PaperServiceComponent.listPaperById(this.state.paperId).then(res =>{
            ...
            this.setState({
                editorId: res.data.editor.userId ,price: res.data.price, publishDate: res.data.publishDate, newsData:res.data.newsList
            });
        })
    }
    render() {
                                ...
                                <div className="card-body">
                                    <div className="form-group">
                                        <label className="form-label w-48">
                                            Editor Id:
                                    <input type="text" className="form-control" name="editorId" value={this.state.editorId} readOnly />
                                        </label>&emsp;
                                        <label className="form-label w-48">
                                            Date :
                                    <input type="text" className="form-control" name="publishDate" value={new Date(this.state.publishDate).toLocaleDateString()} readOnly />
                                        </label>
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label w-48">
                                            Price:
                                    <input type="text" className="form-control" name="price" value={this.state.price} onChange={this.myChangeHandler} />
